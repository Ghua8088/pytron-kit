...
class ShortcutManager:
    ...
    def _register_darwin(self, combo: str, callback: Callable):
        ...
        mac_mods = 0
        if modifiers & MOD_CONTROL: mac_mods |= (1 << 0)
        if modifiers & MOD_SHIFT: mac_mods |= (1 << 1)
        if modifiers & MOD_ALT: mac_mods |= (1 << 3)
        if modifiers & MOD_WIN: mac_mods |= (1 << 8)
    ...
    def _start_darwin_loop(self):
        try:
            from Quartz import CGEventTapCreate, kCGSessionEventTap, kCGHeadInsertEventTap, kCGEventKeyDown, kCGEventMaskForAllEvents, \
                               CGEventTapEnable, CFRunLoopAddSource, CFRunLoopGetCurrent, kCFRunLoopCommonModes, \
                               CFRunLoopRun, CGEventGetFlags, CGEventGetIntegerValueField, kCGKeyboardEventKeycode
            ...
            self.logger.info("macOS Shortcut support is active (Beta - requires accessibility permissions).")
    ...
    def _register_windows(self, combo: str, callback: Callable):
        ...
        data = {'id': sid, 'fsModifiers': modifiers, 'vk': vk, 'callback': callback, 'registered': False}
            ...
            if msg.message == WM_HOTKEY:
                ...
                for sid, data in self.shortcuts.items():
                    if not data.get('registered', False):
                        success = user32.RegisterHotKey(None, sid, data['fsModifiers'], data['vk'])
                        if success:
                            data['registered'] = True
                            self.logger.info(f"Registered global shortcut ID {sid}")
                        else:
                            self.logger.error(f"Failed to register ID {sid}. Error: {ctypes.GetLastError()}")
