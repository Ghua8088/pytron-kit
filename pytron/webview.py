...
class Webview:
    def __init__(self, config):
        if not pytron_native:
            raise ImportError("Pytron Native Engine (pyd) is missing. Run build_engine.py.")
        ...
        if not raw_url.startswith(("http:", "https:", "pytron:")):
            ...
            if path_obj.exists():
                 ...
                 final_url = f"https://pytron.localhost/app/{urllib.parse.quote(path_obj.name)}"
            else:
                 # Fallback
                 root_path = str(path_obj.parent)
                 final_url = f"https://pytron.localhost/app/{urllib.parse.quote(path_obj.name)}"
        ...
        self.logger.info(f"Native Engine Init: URL={final_url}, VirtualRoot={root_path}")
        ...
        if config.get("min_size") or config.get("max_size"):
             self.logger.warning("Native Engine: min_size/max_size are not currently supported without rebuild. Ignoring.")
    ...
    def _init_bindings(self):
        ...
        self.bind("pytron_set_slim_titlebar", self.set_slim_titlebar, run_in_thread=False)
        ...
        self._spammy_methods = {"pytron_sync_state", "pytron_serve_asset", "__pytron_vap_get"}
    ...
    def _normalize_to_pytron(self, url):
        ...
        try:
            ...
            self.logger.warning(f"Navigate path {url} is outside protocol root {self.root_path}. Falling back to raw path (likely file://).")
            return str(path_obj)
    ...
    def _apply_ui_settings(self):
        ...
        if self.config.get("default_context_menu") is False:
             js.append("try { document.addEventListener('contextmenu', e => e.preventDefault()); } catch(e) {}")
             
        if js:
            # We use setTimeout to allow parsing to complete if called early
            full_script = "(function(){ setTimeout(() => { " + " ".join(js) + " }, 100); })();"
    ...
    def set_always_on_top(self, enable):
        ...
        if call_native:
            ...
            ctypes.windll.user32.SetWindowPos(self.hwnd, z_order, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE)
    ...
    def dialog_open_file(self, *args, **kwargs):
        ...
        if hasattr(self.native, "dialog_open_file"):
             ...
             default_path = kwargs.get("default_path") or (args[1] if len(args) > 1 else None)
             file_types = kwargs.get("file_types") or (args[2] if len(args) > 2 else None)
        ...
        if self._platform and self.hwnd: return self._platform.open_file_dialog(self.hwnd, *args, **kwargs)
        return []
    ...
    def dialog_save_file(self, *args, **kwargs):
        if hasattr(self.native, "dialog_save_file"):
             ...
             return self.native.dialog_save_file(title, default_path, default_name, filters_str)

        if self._platform and self.hwnd: return self._platform.save_file_dialog(self.hwnd, *args, **kwargs)
        return None
    ...
    def dialog_open_folder(self, *args, **kwargs):
        ...
        if self._platform and self.hwnd: return self._platform.open_folder_dialog(self.hwnd, *args, **kwargs)
        return None
    ...
    def message_box(self, *args, **kwargs):
        if hasattr(self.native, "message_box"):
             ...
             if isinstance(style, int):
                 if style & 0x10: level = "error"
                 elif style & 0x30: level = "warning"
             
             return self.native.message_box(title, message, level)

        if self._platform and self.hwnd: return self._platform.message_box(self.hwnd, *args, **kwargs)
        return 0
    ...
    def set_taskbar_progress(self, state="normal", value=0, max_value=100): 
        ...
        if hasattr(self.native, "set_taskbar_progress"):
            s_map = {"normal": 2, "error": 4, "paused": 8, "indeterminate": 1, "none": 0}
    ...
    def system_notification(self, title, message, icon=None): 
        if self._platform and self.hwnd:
            if not icon: icon = self.config.get("icon")
    ...
    def _get_binary_asset(self, key):
        ...
        try:
            ...
            if str(possible_path).startswith(str(self._app_root)) and possible_path.exists() and possible_path.is_file():
                import mimetypes
