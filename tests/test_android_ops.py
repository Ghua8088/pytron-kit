...
def test_sync_android_project_basic(tmp_path, mock_console_sync, mock_log_sync):
    ...
    with patch("pytron.platforms.android.ops.sync.importlib.metadata.packages_distributions", return_value={}), \
         patch("shutil.copytree") as mock_copytree, \
         patch("shutil.copy2") as mock_copy2:
         
        # Mock pytron location detection to avoid looking for system packages
        with patch("pytron.platforms.android.ops.sync.os.path.exists", side_effect=lambda p: True): 
